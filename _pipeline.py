...
from openiti.helper.rgx import auth, book, version, loc, manuscr, transcr
...
location_uri_regex = loc
manuscript_uri_regex = manuscr
transcription_uri_regex = transcr
...
def get_known_collections():
    ...
    for coll in sorted(list(set([re.findall(r"[a-zA-Z]+", id_)[0] for id_ in df["id"]]))):
...
def milestone(text, fn, prev_ms=0, ms_length=300):
    ...
    Returns: 
        tup (str, int)
    """
    #ara_tok = re.compile(r"^[ذ١٢٣٤٥٦٧٨٩٠ّـضصثقفغعهخحجدًٌَُلإإشسيبلاتنمكطٍِلأأـئءؤرلاىةوزظْلآآ]+$")
    ...
    test = re.sub(r" ms([A-Z])?\d+", "", ms_text)
    # test = re.sub(r" +", " ", test)
    if test == text:
        #print("\t\tThe file has not been damaged!")
        ms = re.findall(r"ms[A-Z]?\d+", ms_text)
...
##    if editor:
##        text = re.sub(r"INSERT_EDITOR", editor, text)
...
def check_meta_header(text):
    ...
    try:
        header, text = re.split(r"#META#Header#End#", text, maxsplit=1)
        return header, text
    except:
...
def clean(text, fn, auto=False):
    ...
    if auto:
        ...
        for c in sorted(filtered_chars):
            ...
            for x in re.findall(r"[^%s]{,20}ـــ%sـــ+[^%s]{,20}"%(c,c,c), text)[:10]:
                print(x)
                print("-"*30)
...
def check_collection(fn):
    ...
    """
    try:
        collection = re.split(r"\d", fn.split(".")[2])[0]
    except:
        return False
...
def check_uri(folder, fn):
    ...
    """
    if re.match(version_uri_regex, fn) or re.match(transcription_uri_regex, fn):
    ...
    if corrected_fn.startswith("MS"):
        if not re.match(location_uri_regex, corrected_fn):
            print("Error in location URI:", corrected_fn)
            print("Please check the author URI has the following structure:")
            print("1. starts with 'MS' followed by 4 digits")
            print("2. City name starts with a capital letter")
            print("3. City and collection names do not contain any special characters")
            if re.findall(r"[^a-zA-z0-9]", corrected_fn.split(".")[0]):
                print("-> Invalid characters:")
                print(re.findall(r"[^a-zA-z0-9]", corrected_fn.split(".")[0]))
            print()
            corrected_fn = input("Write your corrected filename here:")
        if not re.match(manuscript_uri_regex, corrected_fn):
            print("Error in manuscript URI:", corrected_fn)
            print("Please check the shelfmark:")
            print("1. It should start with a capital letter")
            print("2. It should not contain any special characters apart from 0-9, a-z, A-Z and underscore")
            if re.findall(r"[^a-zA-z0-9_]", corrected_fn.split(".")[1]):
                print("-> Invalid characters:")
                print(re.findall(r"[^a-zA-z0-9_]", corrected_fn.split(".")[1]))
            print()
            corrected_fn = input("Write your corrected filename here:")
        if not re.match(version_uri_regex, corrected_fn):
            print("Error in transcription URI:", corrected_fn)
            print("Example of a correct transcription URI: MS0001PrincetonUL.Garett247B.SK202503-per1")
            ...
            print("4. A number that refers to the type of transcription (by default: 1)")
            corrected_fn = input("Write your corrected filename here:")
    ...
    if re.match(version_uri_regex, corrected_fn) or re.match(transcription_uri_regex, corrected_fn):
        ...
        if not confirm.lower() == "n":
            ...
            old_yml_fp = re.sub(r"(-(?:[a-z]{3}\d+)+).*", r"\1.yml", old_fp)
            new_yml_fp = re.sub(r"(-(?:[a-z]{3}\d+)+).*", r"\1.yml", new_fp)  
            try:
...
def check_yml_file(yml_fp, yml_type):
    ...
    uri_key = [k for k in yml_d if "#URI#" in k][0]
...
def check_yml_files(fp):
    ...
    """
    yml_ok = True
    fp_without_ext, lang, ext = re.findall(r"(.+?)(-(?:[a-z]{3}\d+)+)(.*)", fp)[0]
    fn = os.path.split(fp)[1]
    if fn.startswith("MS"):
        ## transcription yml file:
        yml_fp = fp_without_ext + lang + ".yml"
        if os.path.exists(yml_fp):
            yml_ok = check_yml_file(yml_fp, "TRNS")
        ## manuscript yml file:
        yml_fp = ".".join(fp_without_ext.split(".")[:-1]) + ".yml"
        if os.path.exists(yml_fp):
            yml_ok = check_yml_file(yml_fp, "MS")
        # location yml file:
        yml_fp = fp_without_ext.split(".")[0] + ".yml"
        if os.path.exists(yml_fp):
            yml_ok = check_yml_file(yml_fp, "LOC")
    else:
        ## version yml file:
        yml_fp = fp_without_ext + lang + ".yml"
        if os.path.exists(yml_fp):
...
def main(folder, out_folder, start_date=0, end_date=10000, 
         auto_clean=True, silent=False, do_not_move_regex=r"[Nn]oorlib", 
         ms_pattern=r" *ms[A-Z]?\d+", ms_length=300, non_25Y_folder=None,
    ...
    for fn in sorted(os.listdir(folder)):
        ...
        lang_code = re.findall(r".+-([a-z]{3})", fn)[0]
...
if __name__ == "__main__":
    ...
    main(folder=r"C:\Users\peter.verkinderen\Documents\OpenITI\PERbarzakh",
         out_folder=r"C:\Users\peter.verkinderen\Documents\OpenITI\all_repos\25Y_repos",
